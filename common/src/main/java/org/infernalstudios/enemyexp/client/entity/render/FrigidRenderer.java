...
public class FrigidRenderer extends GeoEntityRenderer<FrigidEntity> {
    public FrigidRenderer(EntityRendererProvider.Context renderManager) {
        ...
        addRenderLayer(new ItemArmorGeoLayer<>(this) {
            @Nullable
            @Override
            protected ItemStack getArmorItemForBone(GeoBone bone, FrigidEntity animatable) {
                return switch (bone.getName()) {
                    case "armorhead" -> this.armorStackForBone(animatable, EquipmentSlot.HEAD);
                    case "armorbody", "armorrightarm", "armorleftarm" -> this.armorStackForBone(animatable, EquipmentSlot.CHEST);
                    default -> null;
                };
            }

            @Override
            protected @NotNull EquipmentSlot getEquipmentSlotForBone(GeoBone bone, ItemStack stack, FrigidEntity animatable) {
                return switch (bone.getName()) {
            ...
            @Override
            protected @NotNull ModelPart getModelPartForBone(GeoBone bone, EquipmentSlot slot, ItemStack stack, FrigidEntity animatable, HumanoidModel<?> baseModel) {
                return switch (bone.getName()) {
                    case "armorhead" -> baseModel.head;
                    case "armorbody" -> baseModel.body;
            ...
            private ItemStack armorStackForBone(FrigidEntity entity, EquipmentSlot slot) {
        ...
        addRenderLayer(new BlockAndItemGeoLayer<>(this) {
            @Nullable
            @Override
            protected ItemStack getStackForBone(GeoBone bone, FrigidEntity animatable) {
                return switch (bone.getName()) {
            ...
            @Override
            protected ItemDisplayContext getTransformTypeForStack(GeoBone bone, ItemStack stack, FrigidEntity animatable) {
                return switch (bone.getName()) {
            ...
            @Override
            protected void renderStackForBone(PoseStack poseStack, GeoBone bone, ItemStack stack, FrigidEntity animatable, MultiBufferSource bufferSource, float partialTick, int packedLight, int packedOverlay) {
